...
- [x] `TuiApp.setTheme()` 触发全局重渲染（标记 dirty → 下一帧重绘）
- [x] Widget 颜色延迟解析：`emitDrawCommands()` 时从当前主题 + 本地覆盖计算
...
- [x] 内置 `catppuccinLatte`（亮色）主题
- [x] 内置 `nord`（暗色）主题
- [x] 内置 `highContrast`（高对比度）主题
...
| `packages/core/src/theme/themes.ts` | 内置主题定义（`catppuccinMocha`、`catppuccinLatte`、`nord`、`highContrast`） |
