...
- [x] **D-1** Isolate `NativeBackend` from main barrel — remove `NativeBackend` export from `packages/core/src/index.ts` so browser bundlers never touch `bun:ffi`. `HtmlBackend` and `WasmModule` remain in the main barrel (no `bun:ffi` dependency, safe for both environments). `createBackend()` in `platform/browser.ts` keeps throwing — users must pass `backend` explicitly. `NativeBackend` is only reachable via `@buntui/core/platform` conditional export (native path, where `bun:ffi` exists). Added `"browser"` field in `package.json` to redirect `dist/platform/native.js` → `dist/platform/browser.js` for bundler resolution. No separate `@buntui/html` package needed; xterm.js is a peer dependency
- [x] **D-2** Connect existing `playground-web/` to `HtmlBackend` — wired up a demo app that loads WASM, creates `HtmlBackend`, and renders a BuntUI app into the xterm.js terminal. Programmatic widget API (no SFC compiler needed for browser)
- [x] **D-3** Verify browser bundling with Vite — confirmed `@buntui/core` main entry does not pull in `bun:ffi`. Production build of `playground-web/` produces 457KB JS bundle with zero `bun:ffi` references. Only benign warnings for `node:process`/`node:child_process` in `SystemClipboard.js` (auto-externalized by Vite)
- [x] **D-4** Configure `playground-web/vite.config.ts` for WASM — `server.fs.allow: ['..']` for monorepo access, `optimizeDeps.exclude: ['@buntui/core']`, WASM served from `public/` via `copy-wasm` script
- [x] **D-5** Guard `ptr()` in browser platform — logs a one-time console warning instead of silently returning 0, to catch accidental FFI pointer usage in browser path
- [x] **D-6** WASM binary packaging — `copy-binary.ts` copies `.wasm` to `packages/core/dist/buntui.wasm` alongside native-platforms. `@buntui/core/wasm` sub-path export added in `package.json`. `playground-web` uses `copy-wasm` script to copy from native-platforms to `public/`
