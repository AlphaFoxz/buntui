...
const inputFeedback = computed(
  () => `Username: [${inputValue.value}] (${inputValue.value.length} chars)`,
);
const passwordFeedback = computed(
  () =>
    `Password: ${'*'.repeat(passwordValue.value.length)} (${passwordValue.value.length} chars)`,
);
const limitedFeedback = computed(
  () =>
    `Limited: [${limitedValue.value}] (${limitedValue.value.length}/10 chars)`,
);
const textareaFeedback = computed(
  () =>
...
const textareaPreview = computed(() => {
  ...
  return preview.length > 0
    ? `Preview: ${preview}${textareaValue.value.length > 60 ? '...' : ''}`
    : 'Preview: (empty)';
});
