import type {DrawListBuffer, TuiWidgetRect} from '@buntui/core';
...
export class MatrixWidget extends TuiWidgetEntity {
  ...
  override updateRect(rect: Partial<TuiWidgetRect>): void {
    if (rect.rectWidth !== undefined) this.#rectWidth = rect.rectWidth;
    if (rect.rectHeight !== undefined) this.#rectHeight = rect.rectHeight;
  }
  ...
  override emitDrawCommands(buffer: DrawListBuffer): void {
    ...
    for (let col = 0; col < this.#columns.length; col++) {
      ...
      const cx = absX + col;
      const headY = column.headY;
      const trailLen = column.trailLength;

      for (let t = 0; t < trailLen; t++) {
        const cy = absY + headY - t;
        if (cy < absY || cy >= absY + h) continue;

        const isLead = t === 0;
        const fgRgba = isLead
          ? this.#colorScheme.leadRgba
          : (this.#gradientLut[Math.min(t, maxLen - 1)] ?? this.#colorScheme.trailRgba);
