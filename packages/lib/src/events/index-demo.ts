    ...
    const consume = () => {
      ...
      if (eventSlotPtr !== null) {
        try {
          ...
          const payloadLength = headerView.getUint16(2, true);
          // Const sequence = headerView.getBigUint64(4, true);

          // 读取负载数据
          const payloadBuf = toArrayBuffer(eventSlotPtr, 12, payloadLength);

          // 解析成 JSON
          const schema = schemaRegistry.get(eventType as EventType);
          if (schema) {
            const jsonData = schema.parse(payloadBuf) as Record<string, any>;
...
export class EventBusProducer {
  ...
  emitUserLogin(userId: bigint, timestamp: bigint) {
    ...
    const view = new TuiDataView(buf);
