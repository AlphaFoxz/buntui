<template>
    ...
    >
        ...
        <Box width="100%" :border="false" direction="vertical" :gap="1">
            <Input
                width="100%"
                v-model="projectName"
            ...
            <Text width="100%" :height="1" value="Tab/↑↓ navigate · Enter create · Esc cancel" />
        </Box>
    </Box>
</template>
...
const initialName = argvName && !argvName.startsWith('-') && !['dev', 'build', 'wasm'].includes(argvName)
    ? argvName
    : 'my-buntui-app'
const projectName = ref(initialName)
